...
public class DriveVehicle : MonoBehaviour
{
    ...
    {
        if(characterComponents.myThirdPersonUserControl)
        characterComponents.myThirdPersonUserControl.enabled = active;
        if(characterComponents.myThirdPersonCharacter)
        characterComponents.myThirdPersonCharacter.enabled = active;
        if(characterComponents.myNavMeshObstacle)
        characterComponents.myNavMeshObstacle.enabled = active;
        if(characterComponents.myRigidbody)
        characterComponents.myRigidbody.isKinematic = !active;
        if(characterComponents.myCollider)
        characterComponents.myCollider.enabled = active;
        if (characterComponents.characterController) 
        characterComponents.characterController.enabled = active;
    }
    ...
    public void GetinCar()
    {
        ...
        switch (playerCarStatus)
        {
            ...
            case PlayerCarStatus.Sit:
                ...
                AIContoller.manager.myPlayerCamera.SetActive(false);
                AIContoller.manager.doorParticle.SetActive(false);
            ...
            case PlayerCarStatus.CloseDoor:
                ...
                AIContoller.manager.myPlayerCamera.SetActive(true);
                AIContoller.manager.doorParticle.SetActive(true);
